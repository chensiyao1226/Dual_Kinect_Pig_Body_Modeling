...
//double radius_normal;// = 50;//大一些好
//double radius_feature;// = 40;
const double max_sacia_iterations = 200;
//double min_correspondence_dist;// = 0.01;//小一些好
const double max_correspondence_dist = 500;
...
void sac_ia_align(PointCloud::Ptr source, PointCloud::Ptr target, PointCloud::Ptr finalcloud, Eigen::Matrix4f &init_transform,
	double radius_normal, double radius_feature, int max_sacia_iterations, double min_correspondence_dist, double max_correspondence_dist)
{
...
*/
double pairAlign(const PointCloud::Ptr cloud_src, const PointCloud::Ptr cloud_tgt, PointCloud::Ptr output, Eigen::Matrix4f &final_transform, bool downsample = false, double radius_icp=50)
{
	//
	...
	norm_est.setSearchMethod(tree);
	norm_est.setKSearch(radius_icp);
	...
	//reg.setEuclideanFitnessEpsilon(1);////////1
	// Set the maximum distance between two correspondences (src<->tgt) to 10cm
	// Note: adjust this based on the size of your datasets
	reg.setMaxCorrespondenceDistance(100);//100
	...
	// Get the transformation from target to source    matrix
	...
	*/
	...
	for (size_t i = 1; i < data.size(); ++i)
	{
		...
		*init_result = *target;
		...
		//sac_ia粗配准
		if (sacia)
			sac_ia_align(source, target, init_result, init_transform, radius_normal, radius_feature, max_sacia_iterations, min_correspondence_dist, max_correspondence_dist);
		//ICP精配准
		FitnessScore = pairAlign(source, init_result, temp, pairTransform, true, radius_icp);
		pairTransform *= init_transform;

		pcl::transformPointCloud(*temp, *result, GlobalTransform);
		*final += *result;

		GlobalTransform = GlobalTransform * pairTransform;//迭代乘以转换矩阵
